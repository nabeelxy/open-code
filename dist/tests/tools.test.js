...
describe('bash tool', () => {
    ...
    it('returns stderr merged with stdout', async () => {
        const result = await bashTool.execute({ command: 'echo out && echo err >&2' }, autoCtx);
    ...
    it('respects timeout', async () => {
        const result = await bashTool.execute({ command: 'sleep 10', timeout: 100 }, autoCtx);
        assert.equal(result.isError, true);
...
describe('write_file tool', () => {
    it('creates a new file', async () => {
        const result = await writeTool.execute({ path: 'write-test.txt', content: 'hello world' }, autoCtx);
        assert.equal(result.isError, undefined);
    ...
    it('creates parent directories automatically', async () => {
        const result = await writeTool.execute({ path: 'deep/nested/dir/file.txt', content: 'nested content' }, autoCtx);
        assert.equal(result.isError, undefined);
...
describe('edit_file tool', () => {
    ...
    before(async () => {
        await writeFile(join(testDir, filename), `function greet(name: string) {\n  return \`Hello, \${name}!\`\n}\n`, 'utf8');
    });
    it('replaces a unique string', async () => {
        const result = await editTool.execute({ path: filename, old_string: 'greet', new_string: 'sayHello' }, autoCtx);
        assert.equal(result.isError, undefined);
    ...
    it('errors when old_string is not found', async () => {
        const result = await editTool.execute({ path: filename, old_string: 'nonexistent_string_xyz', new_string: 'replacement' }, autoCtx);
        assert.equal(result.isError, true);
        assert.ok(result.content.includes('not found'));
    });
    it('errors on ambiguous match without replace_all', async () => {
        ...
        const result = await editTool.execute({ path, old_string: 'foo', new_string: 'bar' }, autoCtx);
        assert.equal(result.isError, true);
    ...
    it('replace_all replaces all occurrences', async () => {
        ...
        const result = await editTool.execute({ path, old_string: 'cat', new_string: 'dog', replace_all: true }, autoCtx);
        assert.equal(result.isError, undefined);
        const read = await readTool.execute({ path }, autoCtx);
...
describe('grep tool', () => {
    ...
    before(async () => {
        await writeFile(join(testDir, filename), 'apple\nbanana\napricot\ncherry\n', 'utf8');
    });
    it('finds matching lines', async () => {
        const result = await grepTool.execute({ pattern: 'ap', path: filename }, autoCtx);
        assert.equal(result.isError, undefined);
        assert.ok(result.content.includes('apple') || result.content.includes('apricot'));
    });
    it('reports no matches cleanly', async () => {
        const result = await grepTool.execute({ pattern: 'xyz123nothere', path: filename }, autoCtx);

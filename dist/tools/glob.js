...
export const globTool = {
    name: 'glob',
    description: 'Find files matching a glob pattern. Returns a sorted list of matching paths relative to the search directory. Use this to explore the project structure or find specific file types.',
    parameters: {
        type: 'object',
        properties: {
            pattern: {
                type: 'string',
                description: 'Glob pattern to match (e.g., "**/*.ts", "src/**/*.{js,ts}", "*.json")',
            },
            cwd: {
                type: 'string',
    ...
    async execute(input, ctx) {
        ...
        const searchDir = input['cwd']
            ? resolve(ctx.workingDir, input['cwd'])
            : ctx.workingDir;
        try {

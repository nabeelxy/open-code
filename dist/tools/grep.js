...
export const grepTool = {
    name: 'grep',
    description: 'Search for a regex pattern in files. Uses ripgrep (rg) when available, otherwise falls back to grep. Returns matching lines with file paths and line numbers.',
    parameters: {
        type: 'object',
        properties: {
            pattern: {
    ...
    async execute(input, ctx) {
        ...
        const searchPath = input['path']
            ? resolve(ctx.workingDir, input['path'])
            : ctx.workingDir;
        ...
        try {
            ...
            const footer = lines.length > MAX_OUTPUT_LINES

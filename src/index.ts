...
async function main() {
  ...
  if (opts.auto) config.permissionMode = 'auto'
  if (opts.autoMode) config.permissionMode = 'permissive'
  ...
  try {
    ...
    console.error(chalk.red(`Provider error: ${(err as Error).message}`))
    process.exit(1)
  }
  ...
  const confirm = async (prompt: string): Promise<boolean> => {
    if (config.permissionMode === 'auto') return true
...
function handleSlashCommand(input: string, messages: Message[], rl: readline.Interface) {
...
function buildSystemPrompt(systemContext: string, extra?: string): string {

...
class TestableOpenAIProvider extends OpenAIProvider {
  convert(messages: Message[]) {
    ...
    return (this as unknown as { toOpenAIMessages(m: Message[]): unknown[] }).toOpenAIMessages(
      messages,
    )
  }
}
...
describe('OpenAIProvider message conversion', () => {
  it('converts a plain string user message', () => {
    ...
    const result = provider.convert(messages) as Array<{ role: string; content: string }>
  ...
  it('converts tool_result blocks to tool role messages', () => {
    ...
    const result = provider.convert(messages) as Array<{
      role: string
      tool_call_id?: string
      content?: string
    }>
  ...
  it('converts tool_use blocks to assistant tool_calls', () => {
    ...
    const args = JSON.parse(result[0]?.tool_calls?.[0]?.function.arguments ?? '{}') as {
      path: string
    }
  ...
  it('handles mixed user message with text and tool_result', () => {
    ...
    const result = provider.convert(messages) as Array<{ role: string }>

...
export const readTool: Tool = {
  name: 'read_file',
  description:
    'Read the contents of a file. Returns text with line numbers. Use offset and limit to read sections of large files. Always read a file before editing it.',
  parameters: {
    type: 'object',
    properties: {
      path: {
  ...
  async execute(input, ctx) {
    ...
    const requestedOffset = Math.max(0, ((input['offset'] as number | undefined) ?? 1) - 1)
    const requestedLimit = Math.min(
      (input['limit'] as number | undefined) ?? MAX_LINES,
      MAX_LINES,
    )
    ...
    if (stats.size > MAX_FILE_BYTES) {
      return {
        content:
          `File is too large (${Math.round(stats.size / 1024)} KB). ` +
    ...
    const numbered = slice
      .map(
        (line, i) =>
          `${String(requestedOffset + i + 1).padStart(padWidth)} | ${line}`,
      )
      .join('\n')
